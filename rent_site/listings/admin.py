...
class BookingAdmin(admin.ModelAdmin):
    ...
    ordering = ('-created_at',)  # Сортировка по дате создания бронирования (новые сначала)

    readonly_fields = ('created_at',)  # Поле, которое не следует изменять
    fieldsets = (
        (None, {
            'fields': ('user', 'listing', 'start_date', 'end_date', 'status')
        }),
        ('Booking Date', {
            'fields': ('created_at',),
